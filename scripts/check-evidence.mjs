...
const evidenceVersion = '20260921-engineering-audit';
...
async function currentSource(repository, path) {
  const response = await fetch(
    `https://api.github.com/repos/Sintagmatarches/${repository}/contents/${path}?ref=main&v=${evidenceVersion}`,
    {
      headers: { Accept: 'application/vnd.github.raw+json' },
      signal: AbortSignal.timeout(20_000), cache: 'no-store',
    },
  );
  assert.ok(response.ok, `${repository}/${path}: GitHub API HTTP ${response.status}`);
  return response.text();
}

const [waterText, geoText, geoV1Text, olistText, profile, html, readme] = await Promise.all([
  source('helsinki-water-forecasting-optimization', 'artifacts/v1.0.0/metrics.json'),
  source('finland-geospatial-ai', 'artifacts/evaluation/E3-segformer-b0-rgb-test-metrics.json'),
  source('finland-geospatial-ai', 'artifacts/test-metrics.json'),
  source('applied-ai-lab', 'artifacts/metrics.json'),
  currentSource('Sintagmatarches', 'README.md'),
  readFile(new URL('../index.html', import.meta.url), 'utf8'),
  readFile(new URL('../README.md', import.meta.url), 'utf8'),
]);
const water = JSON.parse(waterText);
const geo = JSON.parse(geoText).global;
const geoV1 = JSON.parse(geoV1Text).test_metrics;
...
const claims = [
  ...
  { title: 'Finland Geospatial AI', values: [
    geo.mean_iou.toFixed(4), geo.macro_dice.toFixed(4),
    geoV1.miou.toFixed(4), geoV1.macro_dice.toFixed(4),
  ] },
